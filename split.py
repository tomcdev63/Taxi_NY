...
class Splitter:
    ...
    def extract_data(self, file_name, column_to_split_on, output_folder):
        if output_folder not in os.listdir(f"{self.data_path}/CURATED"):
            self.file_splitter(file_name, column_to_split_on, output_folder)
        else:
            print("Fichiers déjà splitté.")
    ...
    def file_splitter(self, file_name, column_to_split_on, output_folder):
        ...
        with open(f"{self.data_path}/RAW/{file_name}", encoding='utf-8') as file:
            ...
            for row in tqdm(file_dict, ncols=200):
                file_column = row[column_to_split_on][0:4]
